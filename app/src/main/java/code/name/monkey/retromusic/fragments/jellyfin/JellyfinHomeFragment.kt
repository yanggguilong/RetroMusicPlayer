...
class JellyfinHomeFragment : Fragment(R.layout.fragment_jellyfin_library_new) {
    ...
    private fun setupUI() {
        ...
        // 设置专辑网格适配器
        albumCardAdapter = JellyfinAlbumCardAdapter {
            navigateToAlbumDetails(it.id)
        }
    ...
    private fun shuffleAll() {
        requireContext().showToast("随机播放所有音乐")
    }

    private fun navigateToAlbumDetails(albumId: String) {
        // 使用父 Fragment 的导航控制器
        findNavController().navigate(
            R.id.action_jellyfin_main_to_jellyfinAlbumDetailsFragment,
